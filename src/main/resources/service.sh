...
function start() {
  ...
  nohup java $JAVA_OPTS --class-path=./lib/:$JAVA_HOME/jre/lib/ext -Djava.security.egd=file:/dev/./urandom -jar -Dspring.config.location=application.yml $jpsCommand.jar >/dev/null 2>&1 &
